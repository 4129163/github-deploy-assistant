...
class InstallCommand {
  ...
  setupCLI() {
    ...
    this.program = new Command('install');
    
    this.program
      .description('安装GADA系统服务（支持开机自启和崩溃重启）')
      .option('-u, --user <user>', '指定运行用户 (仅Linux)')
      .option('-s, --service-name <name>', '指定服务名称', 'gada')
      .option('-d, --display-name <name>', '指定显示名称', 'GitHub Deploy Assistant')
      .option('-i, --install-dir <dir>', '自定义安装目录', process.cwd())
      .option('--no-service', '仅安装，不注册系统服务')
      .option('--skip-deps', '跳过依赖安装')
      .option('--skip-ai', '跳过AI配置')
  ...
  async interactiveInstall() {
    console.log(chalk.cyan.bold('🚀 GADA 系统服务安装程序\n'));
